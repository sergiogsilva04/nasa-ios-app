...
class EpicViewModel: ObservableObject {
        ...
        Task{
            do {
                ...
                DispatchQueue.main.async {
                    ...
                    self.previousImageIndex = self.epic!.count-1
                }
    ...
    func getPictures() async throws {
        let (data, response) = try await URLSession.shared.data(for: URLRequest(url: URL(string: "https://epic.gsfc.nasa.gov/api/natural/date/\(currentDate.format(format: "yyyy-MM-dd"))")!))
        
        guard (response as? HTTPURLResponse)?.statusCode == 200 else {
            print("Error loading epic")
            return
        }
